...
(function () {
  ...
  var meta = function () {
    var metaTable = {};
    var metaId = '_meta_' + Math.random().toString(36).substr(2, 8);
    return function (obj) {
      var getAll = function getAll(create) {
        var id = obj[metaId];
        if (create) {
          if (!id) {
  ...
  var GlaceFactory = function GlaceFactory(PARSER) {
     ...
     **/
    var append = function append(parentNode, childTemplate, state, util) {
      ...
      var rendered = childTemplate(doc, state, util);
    ...
    var condition = function condition(trueTemplate, falseTemplate, _condition) {
      ...
      return function (doc, state, util) {
        var ins = NOP();
        ...
        var update = function update(newState) {
          ...
          if (newValue !== value) {
            ...
            ins = newValue ? trueTemplate(doc, newState, util) : falseTemplate(doc, newState, util);
            collect(ins.output, function (node) {
    ...
    var makeDOMGenerators = function makeDOMGenerators(nodes, filename) {
      ...
      collect(nodes, function (node) {
        ...
        if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          generators.push(function (d, s, util) {
            ...
            collect(events, function (handler) {
              ...
              var events = m.get('events');
              var ev = [handler.name, function (event) {
    ...
    var handler = function handler(name, propNames, template) {
      ...
      tagHandlers[name] = function (props) {
        ...
        return function (doc, state, util) {
          var ret = tpl(doc, adjustState(state), util);
    ...
    var fetchTemplate = function fetchTemplate(url, loadingClass) {
      ...
      template = function template(d, s, util) {
        if (realTemplate) {
        ...
        promise.then(function (realTemplate) {
          var rendered = realTemplate(d, currentState, util);
  ...
  (function () {
    try {
      ...
      collect(document.querySelectorAll('script[id][type="' + CONTENT_TYPE + '"]'), function (scr) {
        Glace.register(scr.id, scr.getAttribute('props'), function () {
      ...
      collect(document.querySelectorAll('meta[glace-template]'), function (meta) {
        ...
        Glace.register(id, meta.getAttribute('props'), function () {
          return Glace.fetch(url, meta.getAttribute('loading-class'));
        });
      });
    }
  })();
