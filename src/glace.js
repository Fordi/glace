((global, factory)=>{
  ...
  let compile = (tpl, filename) => {
    ...
    let err = dom.querySelector(PARSERERROR);
    if (err) {
  ...
  each.call(document.querySelectorAll(`script[id][type="${CONTENT_TYPE}"]`), el => {
    registerTemplate(scr.id, scr.getAttribute('props'), () => compile(scr.textContent, '#' + scr.id));
  });
  each.call(document.querySelectorAll('meta[glace-template]'), meta => {
    ...
    registerTemplate(id, meta.getAttribute('props'), () => fetchTemplate(url, meta.getAttribute('loading-class')));
  });
  return exports;
});
