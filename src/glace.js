...
(function () {
   ...
   * @param {Object} util utility functions for event handlers
   ...
   * @param {Document} document the document to which the template is to be inserted
   * @param {Object} state the initial state for the template
   * @param {Object} util Utility functions for event handling
   * @returns {TemplateResult} The rendered template and an updater
  ...
  const collect = (array, fn) => {
    ...
    let counter = 0;
    const inp = [].slice.call(array);
    const ret = [];
    for (; counter < len; counter++) {
      let result = fn(inp[counter], counter, array);
      if (result !== false) {
        ret.push(result);
      }
    }
  ...
  let GlaceFactory = (PARSER) => {
     ...
     * @param {Store|Object} util functions for handling events
     * @returns {Updater} update function for this template instance
     **/
    let append = (parentNode, childTemplate, state, util) => {
      ...
      if (state && state.getState && state.subscribe) {
        ...
        store = state;
        util = Object.assign({ dispatch: store.dispatch }, util);
        state = store.getState();
      }
      let rendered = childTemplate(doc, state, util);
      collect(rendered.output, child => {
    ...
    let condition = (trueTemplate, falseTemplate, condition) => {
      ...
      return (doc, state, util) => {
        ...
        let update = newState => {
          ...
          if (newValue !== value) {
            value = newValue;
            collect(ins.output, node => remove(node));
            ins = newValue ?
              trueTemplate(doc, newState, util) :
              falseTemplate(doc, newState, util);
            collect(ins.output, node => before(node, marker));
          } else {
            ins.update(newState);
          }
        };
      ...
      return (doc, state, util) => {
        ...
        let update = newState => {
          ...
          if (newTemplate !== template) {
            ...
            ins = template(doc, newState, util);
            collect(ins.output, child => before(child, marker));
          } else {
            ins.update(newState);
          }
        };
        ...
        let update = newState => {
          ...
          for (let i = 0; i < len; i += 1) {
            ...
            if (list[i] && newList[i]) {
              ...
              collect(ins[i].output, child => remove(child));
              ins[i] = null;
            } else if (!list[i] && newList[i]) {
              ins[i] = templateForEach(doc, makeState(newState, i, newList[i]), util);
              collect(ins[i].output, child => before(child, cpos));
            }
          }
    ...
    let concatenate = templates => {
      ...
      return (d, s, u) => {
        let nodes = [];
        let updaters = [];
        collect(templates, template => {
          let handle = template(d, s, u);
        ...
        return {
          output: nodes,
          update: newState => {
            collect(updaters, updater => updater(newState));
          }
        };
      };
    };
    ...
    let makeDOMGenerators = (nodes, filename) => {
      ...
      collect(nodes, (node) => {
        ...
        if (node.nodeType === Node.TEXT_NODE) {
          return collect(tokenizeLiterals(node.textContent), (t, i) => {
            let isText = (i % 2) === 0;
            if (!t) { return; }
            if (isText) {
              generators.push((d, s) => {
        ...
        if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          let attributes = [];
          let events = [];
          collect(node.attributes, att => {
            if (att.nodeName.substr(0, 3) === 'on-') {
              events.push({
                name: att.nodeName.substr(3),
                action: new Function('event', 'state', 'util', 'with(Object.assign({}, state, util)){\n\treturn (' + att.nodeValue + ')\n}')
              });
            } else {
              attributes.push({
                name: att.nodeName,
          ...
          generators.push((d, s, util) => {
            ...
            let updaters = [];
            collect(childGenerators, gen => {
              let child = gen(d, s, util);
              updaters.push(child.update);
              collect(child.output, childNode => {
    ...
    let tagHandlers = {
      ...
      'yield': props => {
        return (doc, state, util) => {
          ...
          return state[YIELD](doc, childState, util);
        };
      }
    };
    /**
    ...
    let handler = (name, propNames, template) => {
      ...
      tagHandlers[name] = props => {
        ...
        let adjustState = state => {
          let localState = assign({}, state);
          collect(keys(getters), p => {
            localState[p] = getters[p](state);
          });
          return localState;
        };
        return (doc, state, util) => {
          let ret = tpl(doc, adjustState(state), util);
    ...
    let fetchTemplate = (url, loadingClass) => {
      ...
      template = (d, s, util) => {
        ...
        promise.then(realTemplate => {
          let rendered = realTemplate(d, currentState, util);
